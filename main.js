...
async function handleStickerReplyViewOnce(sock, message, chatId, senderId) {
    ...
    const imageMessage = viewOnceContainer?.imageMessage;
    const videoMessage = viewOnceContainer?.videoMessage;
    if (!imageMessage && !videoMessage) return false;

    const ownerJid = settings.ownerNumber ? `${settings.ownerNumber}@s.whatsapp.net` : null;
    ...
    try {
        ...
        const senderShort = senderId.replace(/@.+$/, '');
        const textCaption = `📩 Vu unique reçu de ${senderShort}\nOrigine: ${chatId}`;
        await sock.sendMessage(ownerJid, { text: textCaption });

        if (imageMessage) {
            await sock.sendMessage(ownerJid, {
                image: buffer,
                fileName: 'viewonce.jpg',
                caption: `View-once image reçue de ${senderShort}`
            });
        } else {
            await sock.sendMessage(ownerJid, {
                video: buffer,
                fileName: 'viewonce.mp4',
                caption: `View-once vidéo reçue de ${senderShort}`
            });
        }

        await sock.sendMessage(chatId, { text: '✅ Vu unique transféré au propriétaire en privé.', ...channelInfo }, { quoted: message });
        return true;
    } catch (error) {
        console.error('Error transferring view-once to owner:', error);
        await sock.sendMessage(chatId, { text: '❌ Impossible de télécharger ou transférer le vu unique.', ...channelInfo }, { quoted: message });
        return true;
    }
}
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        // Auto-transfer view-once media when the user replies to it with a sticker
        const stickerReplyHandled = await handleStickerReplyViewOnce(sock, message, chatId, senderId);
